      ...
      <main className="flex flex-col md:grid grid-cols-4 w-10/12 m-auto content-start gap-10">
        <aside className="col-span-1  flex flex-col">
          <figure className="">
            <GatsbyImage
              image={
                data.contentfulEntrepreneurs.photoentrepreneur.gatsbyImageData
              }
              alt=""
              className=""
            />
          ...
          <h2 className=" bg-orange-interra text-white text-3xl p-2 mb-5 text-center">
            {data.contentfulEntrepreneurs.nomDeLentrepreneur}
          </h2>
          <nav>
            <ul className="flex flex-col gap-2">
              <li className="flex flex-row gap-2 items-center">
                ...
                <a
                  href={data.contentfulEntrepreneurs.siteInternet}
                  target="_blank"
                  rel="noreferrer"
                >                  <p className=" hover:text-orange-interra hover:decoration-solid">

                  {data.contentfulEntrepreneurs.siteInternet}
                  </p>
                </a>
              </li>
              <li className="flex flex-row gap-2 items-center">
                ...
                <a
                  href={data.contentfulEntrepreneurs.mail}
                  target="_blank"
                  rel="noreferrer"
                >
                  <p className=" hover:text-orange-interra hover:decoration-solid">
                    {" "}
                    {data.contentfulEntrepreneurs.mail}
                  </p>
                </a>
              </li>
            </ul>
            <div className="flex flex-row gap-2 mt-5 items-center">
              <a
                href={data.contentfulEntrepreneurs.facebookbon}
                target="_blank"
                rel="noreferrer"
              >
                <svg
              ...
              <a
                href={data.contentfulEntrepreneurs.instagram}
                target="_blank"
                rel="noreferrer"
              >
                <svg
        ...
        <article className="col-span-3 flex flex-col ">
          <figure className="">
            <GatsbyImage
              image={data.contentfulEntrepreneurs.photoprojet.gatsbyImageData}
              alt=""
              className="w-full"
            />
          </figure>
          <h1 className="text-3xl p-2 bg-orange-clair-interra text-white font-bold w-full mb-10">
            {data.contentfulEntrepreneurs.nomduprojet}
          </h1>
          <div
            className=" description flex flex-col gap-2 "
            dangerouslySetInnerHTML={{
              __html:
                data.contentfulEntrepreneurs.description.childMarkdownRemark
                  .html,
            }}
          />
        </article>
...
export const query = graphql`
  query histoirePageQuery($slug: String) {
    contentfulEntrepreneurs(slug: { eq: $slug }) {
      description {
        childMarkdownRemark {
          html
        }
      ...
      photoprojet {
        gatsbyImageData(height: 240, width: 800)
      }
    }
  }
`;
