      ...
      <main className="flex flex-col md:grid grid-cols-4 w-10/12 m-auto content-start gap-10">
        <aside className="col-span-1  flex flex-col">
          <figure className="">
            <GatsbyImage
              image={
                data.contentfulPartenaires.imagePartenaire.gatsbyImageData
              }
              alt=""
              className=""
            />
          ...
          <h2 className=" bg-orange-interra text-white text-3xl p-2 mb-5 text-center">
            {data.contentfulPartenaires.nomPartenaire}
          </h2>
          <nav>
            <ul className="flex flex-col gap-2">
              <li className="flex flex-row gap-2 items-center">
                ...
                <a
                  href={data.contentfulPartenaires.siteInternet}
                  target="_blank"
                  rel="noreferrer"
                >                  <p className=" hover:text-orange-interra hover:decoration-solid">

                  {data.contentfulPartenaires.siteInternet}
                  </p>
                </a>
              </li>
              <li className="flex flex-row gap-2 items-center">
                ...
                <a
                  href={data.contentfulPartenaires.mail}
                  target="_blank"
                  rel="noreferrer"
                >
                  <p className=" hover:text-orange-interra hover:decoration-solid">
                    {" "}
                    {data.contentfulPartenaires.mail}
                  </p>
                </a>
              </li>
            </ul>
            <div className="flex flex-row gap-2 mt-5 items-center">
              <a
                href={data.contentfulPartenaires.facebook}
                target="_blank"
                rel="noreferrer"
              >
                <svg
              ...
              <a
                href={data.contentfulPartenaires.instagram}
                target="_blank"
                rel="noreferrer"
              >
                <svg
        ...
        <article className="col-span-3 flex flex-col ">
          <div
            ...
            dangerouslySetInnerHTML={{
              __html:
                data.contentfulPartenaires.description.childMarkdownRemark
                  .html,
            }}
          />
        </article>
...
export const query = graphql`
  query PartenairepageQuery($slug: String) {
    contentfulPartenaires(slug: { eq: $slug }) {
      description {
        childMarkdownRemark {
          html
        }
